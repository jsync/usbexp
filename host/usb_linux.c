 ...
 *  usb_linux.c
 *
 * Derived from usb_linux.c from adb
 *
...
pthread_mutex_t usb_lock = PTHREAD_MUTEX_INITIALIZER;
...
static int known_device(const char *dev_name)
{
	...
	for(usb = handle_list.next; usb != &handle_list; usb->next) {
		...
		(const char *, unsigned char, unsigned char, int, int, unsigned))
{
	...
	while((de =readdir(busdir)) != 0) {
		...
		snprintf(busname, sizeof(busnname), "%s/%s", base, de->d_name);
		...
		while((de = readdir(devdir))) {
			...
			//loop through all the descriptors and look for the USBEXP interface
			while (bufptr < bufend) {
				...
				if(type == USB_DT_INTERFACE) {
					...
					DBGX("bInterfaceClass: %d, bInterfaceSubClass: %d,"
							...
							interaface->bInterfaceProtocol, interface->bNumEndpoints);

					if(interface->bNumEndpoints == 2 &&
							is_usbexp_interface(vid, pid, interface->bInterfaceClass,
						...
						if (ep1->EndPointAddress & USB_ENDPOINT_DIR_MASK) {
...
int usb_write(usb_handle *h, const void *_data, int len)
{
	...
	if(h->zero_mask) {
		/* if we need 0-markers and out transfer
...
int usb_read(usb_handle *h, void *_data, int len)
{
	...
	while(len > 0) {
		...
		D("[ usb_read %d fd = %d], fname = %s\n", xfer, h->desc, h->fname);
		n = usb_bulk_read(h, data, xfer);
		D("[ usb_read %d ] = %d, fname = %s\n", xfer, n, h->fname);
		if(n != xfer){
			if((errno == ETIMEOUT) && (h->desc != -1)) {
				D("[ timeout ]\n");
				if(n > 0) {
					data += n;
					len -= n;
...
void usb_kick(usb_handle *h)
{
	...
	if(h->dead == 0) {
		...
		if(h->writeable) {
			 ...
			 * but this ensures that a read blocked on REAPURB
			 * will get unblocked
			 */
			...
			/* Need to define for our use and remove this comment */
							...
							int interface, int serial_index, unsigned zero_mask)
{
	...
	/* Since Linux will not reassign the device ID (and dev name)
	...
	pthread_cond_init(&usb->notify, 0);
	pthread_mutex_init(&usb->lock, 0);
	...
	if(usb->desc < 0) {
		...
		usb->writeable = 0;
		D("[ usb open read_only %s fd = %d]\n", usb->fname, usb->desc);
	} else {
		D("[ usb open %s fd = %d]\n", usb->fname, usb->desc);
		n = ioctl(usb->desc, USBDEVFS_CLAIMINTERFACE, &inteface);
		if (n != 0) goto fail;
	}
	...
	if(serial_index) {
		...
		for(i = 1; i <= languageCount; i++) {
			...
			if(result > 0) {
				int i;
				// skip first word, and copy the rest to the serial strings changing shorts to bytes.
				result /= 2 ;
				for(i = 1;i < result; i++)
					serial[i-1] = buffer[i];
				serial[i-1] = 0;
	...
	usb->next->prev = usb;
	pthread_mutex_unlock(&usb_lock);
	/* need to define for our use and remove the comment */
...
void usb_init()
{
	...
	if(pthread_create(tid, &attr, device_poll_thread, NULL)) {
